...
@php
    ...
    $qInfo    = $quarterLabels[$quarter] ?? $quarterLabels[1];
    $qLabel   = $qInfo[0];
    $qStart   = $qInfo[1];
    $qEnd     = $qInfo[2];
    // Due date: last day of the month after the quarter ends
    $dueMonths = [1 => 4, 2 => 7, 3 => 10, 4 => 1];
    $dueM   = $dueMonths[$quarter] ?? 4;
    $dueY   = $quarter == 4 ? $year + 1 : $year;
    $due    = date('m/d/Y', mktime(0, 0, 0, $dueM, 25, $dueY));

    // Quarterly remittance summary (simplified — monthly 0619-E remittances presumed equal to total)
    $monthlyRemittances = $totalTaxWithheld; // placeholder until 0619-E integration
    $taxStillDue  = 0;
    $taxCredits   = $taxCredits ?? 0;
    $netTaxDue    = max(0, $taxStillDue - $taxCredits);
    $surcharge    = 0;
    $interest     = 0;
    $compromise   = 0;
    $totalPenalty = 0;
...
<div class="bir-page" id="page1">
    ...
    <div class="bir-topbar">
        ...
        <div class="bir-formtitle">
            ...
            <div class="bir-formname">
                ...
                <div style="font-size:22px;font-weight:bold;line-height:1;">1601-EQ</div>
                <div style="font-size:8px;">September 2005 (ENCS)</div>
                <div style="font-size:8px;">Page 1</div>
            </div>
            <div class="bir-formsubtitle">
                <div style="font-size:11px;font-weight:bold;">Quarterly Remittance Return</div>
        ...
        <div class="bir-formno-box">
            <div style="font-size:7px;text-align:right;">1601-EQ 09/05ENCS P1</div>
        </div>
    </div>

    {{-- Row 1: Quarter / Amended / ATC / Tax Type --}}
    <table class="bir-table">
        <tr>
            <td class="bir-cell" style="width:28%">
                <div class="bir-label">1 For the Quarter (Q/YYYY)</div>
                <div class="bir-value">Q{{ $quarter }}/{{ $year }} &nbsp; <span style="font-size:7px;font-weight:normal;">({{ $qStart }}–{{ $qEnd }})</span></div>
            </td>
            ...
            <td class="bir-cell" style="width:16%">
                ...
                <div class="bir-value font-bold">QE</div>
            </td>
            <td class="bir-cell" style="width:16%">
    ...
    <table class="bir-table">
        <tr>
            ...
            <td class="bir-cell bir-item-label">Total Taxes Withheld for the Quarter (from QAP)</td>
            <td class="bir-cell bir-amount"><div class="bir-amount-box">{{ $fmt($totalTaxWithheld) }}</div></td>
        </tr>
        <tr>
            <td class="bir-cell bir-item-num">2</td>
            <td class="bir-cell bir-item-label">Less: Monthly Remittances Made (BIR Form 0619-E) During the Quarter</td>
            <td class="bir-cell bir-amount"><div class="bir-amount-box">{{ $fmt($monthlyRemittances) }}</div></td>
        </tr>
        <tr class="bir-row-bold">
            <td class="bir-cell bir-item-num">3</td>
            <td class="bir-cell bir-item-label">Tax Still Due (Item 1 Less Item 2)</td>
            <td class="bir-cell bir-amount"><div class="bir-amount-box bir-amount-total">{{ $fmt($taxStillDue) }}</div></td>
        </tr>
        <tr>
        ...
        <tr class="bir-row-bold">
            ...
            <td class="bir-cell bir-item-label">Net Tax Due (Item 3 Less Item 4)</td>
        ...
        <tr>
            <td class="bir-cell bir-item-num">6A</td>
        ...
        <tr>
            <td class="bir-cell bir-item-num">6B</td>
        ...
        <tr>
            <td class="bir-cell bir-item-num">6C</td>
        ...
        <tr class="bir-row-bold">
            ...
            <td class="bir-cell bir-item-label">Total Amount Due (Item 5 Plus Items 6A, 6B &amp; 6C)</td>
...
{{-- Part IV: Quarterly Alphalist of Payees (QAP) — screen only supporting schedule --}}
<div class="no-print mt-6">
    <div class="card">
        <div class="card-header">
            <h3 class="text-sm font-semibold text-secondary-900">
                Part IV – Quarterly Alphalist of Payees (QAP)
                &nbsp;<span style="font-size:11px;font-weight:normal;">Q{{ $quarter }} {{ $year }} ({{ $qStart }}–{{ $qEnd }})</span>
            </h3>
            <span class="text-xs text-secondary-400 ml-2">(Supporting document — not printed with the official form)</span>
        </div>
