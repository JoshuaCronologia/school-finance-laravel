...
<head>
    <meta charset="utf-8">
    <title>Bank Reconciliation - {{ $account->account_code }}</title>
    <style>
...
<body>
    <div class="header">
        ...
        <h2>{{ $account->account_code }} - {{ $account->account_name }}</h2>
        <div class="date">As of {{ \Carbon\Carbon::parse($asOfDate)->format('F d, Y') }}</div>
    </div>
                    ...
                    <div class="section-body">
                        <table style="width:100%;">
                            <tr>
                                ...
                                <td style="text-align:right; font-weight:bold; font-size:11px;">₱{{ number_format($statementBalance, 2) }}</td>
                            </tr>
                        </table>

                        <div style="margin-top:10px; font-size:9px; font-weight:bold; color:#555;">ADD: Deposits in Transit</div>
                        @forelse($depositsInTransit as $dep)
                        <table style="width:100%;">
                            <tr>
                                <td class="indent" style="font-size:9px;">{{ \Carbon\Carbon::parse($dep->posting_date)->format('m/d') }} {{ $dep->entry_number }}</td>
                                <td style="text-align:right; font-size:9px;">₱{{ number_format($dep->debit, 2) }}</td>
                            </tr>
                        </table>
                        @empty
                        ...
                        <table style="width:100%;"><tr>
                            ...
                            <td class="sub-line" style="text-align:right; font-size:9px; font-weight:bold;">₱{{ number_format($totalDepositsTransit, 2) }}</td>
                        </tr></table>

                        <div style="margin-top:10px; font-size:9px; font-weight:bold; color:#555;">LESS: Outstanding Checks</div>
                        @forelse($outstandingChecks as $chk)
                        <table style="width:100%;">
                            <tr>
                                <td class="indent" style="font-size:9px;">{{ \Carbon\Carbon::parse($chk->posting_date)->format('m/d') }} {{ $chk->entry_number }} {{ $chk->reference_number ? '('.$chk->reference_number.')' : '' }}</td>
                                <td style="text-align:right; font-size:9px;" class="negative">(₱{{ number_format($chk->credit, 2) }})</td>
                            </tr>
                        </table>
                        @empty
                        <div class="indent" style="font-size:9px; color:#999;">None</div>
                        ...
                        <table style="width:100%;"><tr>
                            <td class="indent sub-line" style="font-size:9px; font-weight:bold;">Total</td>
                            <td class="sub-line" style="text-align:right; font-size:9px; font-weight:bold;" class="negative">(₱{{ number_format($totalOutstandingChecks, 2) }})</td>
                        </tr></table>

                        <table style="width:100%;"><tr>
                            <td class="total-line" style="font-size:11px;">Adjusted Bank Balance</td>
                            <td class="total-line" style="text-align:right; font-size:11px;">₱{{ number_format($adjustedBankBalance, 2) }}</td>
                        </tr></table>
                    </div>
                </div>
            </td>
                    ...
                    <div class="section-body">
                        <table style="width:100%;">
                            <tr>
                                ...
                                <td style="text-align:right; font-weight:bold; font-size:11px;">₱{{ number_format($bookBalance, 2) }}</td>
                            </tr>
                        </table>
                        ...
                        <table style="width:100%;"><tr>
                            ...
                            <td class="total-line" style="text-align:right; font-size:11px;">₱{{ number_format($bookBalance, 2) }}</td>
                        </tr></table>
                    </div>
                </div>
            </td>
    ...
    @else
        <div class="result-box result-unbalanced">UNRECONCILED - Difference: ₱{{ number_format(abs($difference), 2) }}</div>
    @endif
    ...
    <div class="footer">
        School Finance ERP &mdash; Bank Reconciliation Statement &mdash; Printed on {{ $printedAt }}
    </div>
</body>
</html>
