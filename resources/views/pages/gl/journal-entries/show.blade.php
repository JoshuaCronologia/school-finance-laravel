...
<x-page-header :title="$journalEntry->entry_number" subtitle="Journal Entry Details">
    <x-slot name="actions">
        <div class="flex flex-wrap gap-2">
            ...
            {{-- Memorize --}}
            <form action="{{ route('gl.recurring.memorize', $journalEntry) }}" method="POST" class="inline">
                @csrf
                <button type="submit" class="btn-secondary inline-flex items-center gap-1" onclick="return confirm('Are you sure you want to copy this journal entry?')">
            ...
            <a href="{{ route('gl.journal-entries.edit', $journalEntry) }}" class="btn-secondary inline-flex items-center gap-1">
