...
<x-page-header :title="'Request #' . $disbursement->request_number" :subtitle="'Created ' . $disbursement->created_at->format('M d, Y')">
    <x-slot name="actions">
        ...
        <form action="{{ route('ap.disbursements.recurring.memorize', $disbursement) }}" method="POST" class="inline">
            @csrf
            <button type="submit" class="btn-secondary" onclick="return confirm('Are you sure you want to copy this disbursement?')">
