...
<body class="bg-gray-50 antialiased" x-data="{ sidebarOpen: false, sidebarCollapsed: localStorage.getItem('sidebarCollapsed') === 'true' }" x-init="$watch('sidebarCollapsed', val => localStorage.setItem('sidebarCollapsed', val))">
    ...
    <aside :class="[sidebarOpen ? 'translate-x-0' : '-translate-x-full', sidebarCollapsed ? 'lg:w-20' : 'lg:w-64']"
        ...
        <nav class="flex-1 px-3 py-4 space-y-1 overflow-y-auto" :class="sidebarCollapsed ? 'sidebar-mini' : ''">
            ...
            {{-- ACCOUNTS PAYABLE --}}
            @canany(['bill.view', 'disbursement.view'])
            <p class="sidebar-section-title mt-4">Accounts Payable</p>

            {{-- Bills & Disbursements (collapsible) --}}
            <div x-data="{ open: {{ str_starts_with($currentRoute, '/ap/bills') || str_starts_with($currentRoute, '/ap/disbursements') || str_starts_with($currentRoute, '/ap/approval-queue') || str_starts_with($currentRoute, '/ap/payment-processing') || str_starts_with($currentRoute, '/ap/disbursements-recurring') ? 'true' : 'false' }} }">
                <button @click="open = !open" class="sidebar-link w-full justify-between">
                    <span class="flex items-center gap-3">
                ...
                <div x-show="open" x-collapse class="ml-3 mt-1 space-y-0.5 border-l-2 border-gray-200 pl-3">
                    ...
                    <a href="{{ url('/ap/disbursements-recurring') }}" class="sidebar-link text-xs {{ $currentRoute === '/ap/disbursements-recurring' ? 'sidebar-link--active' : '' }}">Recurring Disbursements</a>
            ...
            {{-- ACCOUNTS RECEIVABLE --}}
            @canany(['invoice.view', 'collection.view'])
            ...
            <div x-data="{ open: {{ str_starts_with($currentRoute, '/reports/trial-balance') || str_starts_with($currentRoute, '/reports/balance-sheet') || str_starts_with($currentRoute, '/reports/income-statement') || str_starts_with($currentRoute, '/reports/cash-flow') || str_starts_with($currentRoute, '/reports/general-ledger') || str_starts_with($currentRoute, '/reports/expense-schedule') || str_starts_with($currentRoute, '/reports/fin/') ? 'true' : 'false' }} }">
                ...
                <div x-show="open" x-collapse class="ml-3 mt-1 space-y-0.5 border-l-2 border-gray-200 pl-3">
                    ...
                    <p class="text-xs text-white/40 font-semibold uppercase mt-2 mb-1 pl-1">Cashier Reports</p>
        ...
        @php
            $profileName = auth()->user()->name ?? session('user_info.name') ?? 'User';
        ...
        <div class="border-t border-gray-200 px-4 py-4">
            <div class="flex items-center gap-3" :class="sidebarCollapsed ? 'justify-center' : ''">
                ...
                <form method="POST" action="{{ $logoutAction }}" class="ml-auto" x-show="!sidebarCollapsed">
                    @csrf
            ...
            <div class="flex items-center justify-between px-4 sm:px-6 h-16">
                ...
                <div class="flex items-center gap-3">
                    ...
                    <span class="hidden sm:inline-flex items-center gap-1.5 px-3 py-1 bg-primary-50 text-primary-700 text-xs font-semibold rounded-full">
                        ...
                        SY {{ str_replace('-', '–', \App\Services\FinanceFeeService::currentSchoolYear()) }}
                    </span>
                    ...
                    <div x-data="{ userMenu: false }" class="relative">
                        <button @click="userMenu = !userMenu" class="flex items-center gap-2">
                            <div class="w-8 h-8 bg-primary-600 rounded-full flex items-center justify-center">
                                <span class="text-xs font-semibold text-white">{{ strtoupper(substr(auth()->user()->name, 0, 1)) }}{{ strtoupper(substr(explode(' ', auth()->user()->name)[1] ?? '', 0, 1)) }}</span>
                            </div>
                            <span class="hidden md:block text-sm font-medium text-secondary-700">{{ auth()->user()->name }}</span>
                        ...
                        <div x-show="userMenu" @click.away="userMenu = false"
                            ...
                            <div class="px-4 py-3 border-b border-gray-100">
                                ...
                                <p class="text-xs text-primary-600 font-medium mt-0.5">{{ ucwords(str_replace('_', ' ', auth()->user()->roles->first()->name ?? 'User')) }}</p>
                            </div>
                            {{-- Menu --}}
                            <div class="py-1">
        ...
        function notificationBell() {
            return {
                ...
                init() {
                    ...
                    this._pollTimer = setInterval(() => this.fetchNotifications(), 30000);
                },
                destroy() {
